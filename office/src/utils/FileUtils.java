...
public class FileUtils {
    ...
    public boolean delDir(String path) {
        ...
        if (!file.exists()) {//如果目录不存在返回
            logger.info("Dir Not Exists:" + path);
            return true;
        }
        delDir = file.delete();
        logger.info("Dir Delete Succeed" + path);
        return delDir;
    }
    ...
    public boolean delFile(String path) {
        boolean bool;
       /* loggerInfo.info("delete File :" + path);*/
        File file = new File(path);
        try {
            bool = file.delete();
        } catch (Exception e) {
    ...
    public  Map<String,List<String>> loadDirFiles(String path,String charCode){
        ...
        if (!allFileInSubDir(path)) {
            logger.error("This path contains directories, not all files ");
            return null;
        }
